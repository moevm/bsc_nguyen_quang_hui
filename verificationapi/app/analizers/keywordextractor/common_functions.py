...
def calculateScore(parts_embeddings, keyword_embeddings):
    ...
    for part_emb in parts_embeddings:
        ...
        index += 1
    avg_score = (avg_score/(len(parts_embeddings) if len(parts_embeddings)!=0 else 1)+1)/2
    max_score = (max_score+1)/2
    max_score_index = max_score_index / (len(parts_embeddings) if len(parts_embeddings)!=0 else 1)
