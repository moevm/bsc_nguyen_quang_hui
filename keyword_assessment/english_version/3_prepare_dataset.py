    ...
    for s in data_obj['keywords']:
        if(len(s.strip()) > 0):
            ...
            if(len(s_temp.sentences) > 0):
                # print([w.upos for w in s_temp.sentences[0].words])
                kw_lemma = " ".join([
...
with open('training-cl-25f-tfidf_'+DATASET+'.txt','w') as outfile:
# with open('training-cl-25f-tfidf_nus.txt','w') as outfile:
    json.dump(training_set, outfile)

test_set = test_keywords + test_phrases

print(len(test_set))
with open('test-cl-25f-tfidf_'+DATASET+'.txt','w') as outfile:
# with open('test-cl-25f-tfidf_nus.txt','w') as outfile:
